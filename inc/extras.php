 ...
 * @return array
 */
function smart_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'smart_page_menu_args' );
 ...
 */
function smart_wp_title( $title, $sep ) {
	if ( is_feed() ) {
		return $title;
	}
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title .= " $sep " . sprintf( __( 'Page %s', 'smart' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'smart_wp_title', 10, 2 );
...
add_action( 'wp', 'smart_setup_author' );
